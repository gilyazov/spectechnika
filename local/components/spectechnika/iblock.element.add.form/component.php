...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit_".$componentTemplate."_".$arParams["PREFIX"]]) || !empty($_REQUEST["iblock_apply"])))
	{
        ...
        // hidden inputs ids
        $arParams["PROPERTY_CODES"] = array_merge($arParams["PROPERTY_CODES"], ARRAY_PROPERTY_ID_FIELDS_TYPE_HIDDEN);
		...
		if (empty($arResult["ERRORS"]))
		{
			if (!empty($_REQUEST["iblock_submit_".$componentTemplate."_".$arParams["PREFIX"]]))
			{
